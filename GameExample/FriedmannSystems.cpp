...
//Ќахождение максимально возможного радиуса:
...
SoundSystem::SoundSystem(SelectionSystem* pSelectionSystem, SphericalTransformComponent* pCameraTransform)
{
    ...
    m_pSound = new DynamicSound([this, length, maxRadius](int16_t* data, int sampleRate) { // sampleRate - количество сэмплов
        ...
        //bool willKnockHappenInThisBuffer = (sampleCountForCurrentObject + sampleRate) % mustBePlayedEverySamples;
        ...
        for (int j = 0; j < sampleRate; ++j, ++ptr)
        {
                            ...
                            (playSampleIndex < playSampleCount))
            {
                if (isFirstKnockInThisBuffer)
                {
                    if (needToRecalculateFrequency)
                    {
                        ...
                        mustBePlayedEverySamples = (double)sampleRate / length * mustBePlayedEverySeconds; //вместо 44100 * mustBePlayedEverySeconds; так больше операций, но не зав€заны на 44100
            ...
            *ptr = int16_t(32768 * factor); // PCM 16 bit: -32 767 Е 32 767 
        }
