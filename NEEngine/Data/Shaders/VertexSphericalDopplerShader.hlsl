...
{
	//uint instanceID = 0; //todo: ”Ѕ–ј“№, ќ“Ћјƒ ј!
	...
	float4 position; //итогова€ позици€
	...
	else
	{
		...
		// как это можно проверить: нужно пройти рассто€ние distanceFromPointToCenter от центра в том же самом направлении
		// направление - это вектор в касательном пространстве
		// »де€: найти уравнение пр€мой в гиперсфере
	...
	double distanceCenter; //рассто€ние от наблюдател€ до центра объекта, чтобы дл€ всех его точек было одинаково (чтобы было разное - вз€ть distance)
	distanceCenter = SphericalDistance(float4(0, 0, 0, radius), mul(viewWorld, float4(0, 0, 0, radius)), radius);
	//if (instanceID == 1)
	//	distanceCenter += 3.14159265 * radius;	//€ не прибавл€ю к рассто€нию pi, так как
	//при вычислении разницы это pi устранитс€, и формула ниже примет вид, как внизу
	//lnew - lold = lnew' + pi - lold' - pi = lnew' - lold' = lnew' (1 - rold/rnew)
	//lnew', lold' - без прибавлени€ pi.
