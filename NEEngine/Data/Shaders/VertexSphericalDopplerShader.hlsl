...
{ 
	...
	if (mu < boundary)
		...
		return endRadius + (boundaryRadius - endRadius) * (PI_2 - mu) / startBoundary;
	return 2 * (1 - cos(mu)); 
}
...
)
{
	...
	*/
	...
	else
	{
		...
		// Мы посчитали координаты для радиуса пространства radiusCenterOld.
		// Но остальные координаты, с которыми работаем далее, - для другого радиуса - radius
		// Потому операции типа нахождения расстояния считать некорректно
		// Чтобы решить проблему - нужно домножить position, чтобы сумма квадратов была radius 
		// Видимые размеры от этого не изменятся!
